...
const int MAX_LOG_SIZE = 64*1024*1024;
...
#if _MSC_VER >= 1700
	_snprintf_s(szBuf, buf_size, "%02d-%02d-%02d %02d:%02d:%02d %03d %05d\t",
		st.wYear, st.wMonth, st.wDay, st.wHour, st.wMinute, st.wSecond, st.wMilliseconds, apLog::m_nInstanceID);
#else
	int n = _snprintf(szBuf, buf_size, "%02d-%02d-%02d %02d:%02d:%02d %03d %05d\t",
		st.wYear, st.wMonth, st.wDay, st.wHour, st.wMinute, st.wSecond, st.wMilliseconds, apLog::m_nInstanceID);
#endif
	return string(szBuf);
}
...
bool apLog::init(const _TCHAR* szLogName, apLog::LOG_LEVEL l)
{
	m_nContentSize	= 0;
	m_eLevel		= l;
...
void apLog::print(long instanceID, enum LOG_LEVEL level, const char* szFmt, ...)
{
	try
	{
		...
		static char szBuf[max_buf_size];

		va_list args;
		va_start(args, szFmt);
#if _MSC_VER >= 1700
		_vsnprintf_s(szBuf, max_buf_size, szFmt, args);
#else
		int n = _vsnprintf(szBuf, max_buf_size, szFmt, args);
#endif
		va_end(args);
