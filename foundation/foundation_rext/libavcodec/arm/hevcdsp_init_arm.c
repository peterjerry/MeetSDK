...
void ff_hevc_put_epel_hv_neon_8(int16_t *dst, ptrdiff_t dststride, uint8_t *src,
                                ptrdiff_t srcstride, int height,
                                intptr_t mx, intptr_t my, int width);
void ff_hevc_transform_4x4_add_neon(uint8_t *_dst, int16_t *coeffs,
                                    ptrdiff_t stride, int col_limit);
void ff_hevc_transform_8x8_add_neon(uint8_t *_dst, int16_t *coeffs,
                                    ptrdiff_t stride, int col_limit);
void ff_hevc_transform_16x16_add_neon(uint8_t *_dst, int16_t *coeffs,
                                    ptrdiff_t stride, int col_limit);
void ff_hevc_transform_32x32_add_neon(uint8_t *_dst, int16_t *coeffs,
                                    ptrdiff_t stride, int col_limit);

int ff_hevc_put_qpel_uw_pixels_neon_8(int16_t *dst, ptrdiff_t dststride, uint8_t *_src, ptrdiff_t _srcstride,
                                   int width, int height, int16_t* src2, ptrdiff_t src2stride);
void hevc_sao_band_filter_neon(uint8_t *_dst, uint8_t *_src, ptrdiff_t stride, SAOParams *sao,
...
static av_cold void hevcdsp_init_neon(HEVCDSPContext *c, const int bit_depth)
{
    ...
    if (bit_depth == 8) {
        ...
        c->sao_band_filter    = hevc_sao_band_filter_neon;
	...
	//Fixme compilation error in transform optimizations
        ...
        c->transform_add[0]            = ff_hevc_transform_4x4_add_neon;
        c->transform_add[1]            = ff_hevc_transform_8x8_add_neon; 
        c->transform_add[2]            = ff_hevc_transform_16x16_add_neon;
        c->transform_add[3]            = ff_hevc_transform_32x32_add_neon;
    }
#endif // HAVE_NEON
}
