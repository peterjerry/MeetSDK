...
void apFormatConverter::ff_log_callback(void* avcl, int level, const char* fmt, va_list vl)
{
    AVClass* avc = avcl ? *(AVClass**)avcl : NULL;
	const char * class_name = ((avc != NULL) ? avc->class_name : "N/A");
	
	static char msg[1024] = {0};
	vsnprintf_s(msg, sizeof(msg), fmt, vl);
	static char log[4096] = {0};
#ifdef _MSC_VER
	_snprintf_s(log, 4096, "ffmpeg[%d][%s] %s", level, class_name, msg);
#else
	...
	m_pBsfc_h264(NULL)
{
	...
	av_log_set_callback(apFormatConverter::ff_log_callback);
