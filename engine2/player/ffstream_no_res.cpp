...
AVFormatContext* FFStream::open(char* uri)
{
	...
	int failed_to_open = 0;
	bool isM3u8Broadcast;
	...
	while(1) {
		...
		if (failed_to_open)
			mMovieFile->max_analyze_duration = AV_TIME_BASE * 10; // 10 sec for wrong header ts(more than 5 sec)
		...
		if (mMovieFile->streams[0]->codec->codec_type == AVMEDIA_TYPE_VIDEO) {
			if (0 == mMovieFile->streams[0]->codec->width || 0 == mMovieFile->streams[0]->codec->height) {
				// just try again
				if (!failed_to_open) {
					LOGW("failed to avformat_find_stream_info: %s, retry with 10 sec analyze_duration", uri);
					failed_to_open = 1;
					continue;
				}
			}
