...
#if HAVE_NEON
#ifndef SAO_ARM_NEON
#define SAO_ARM_NEON 1
void hevc_sao_band_filter_0_neon(uint8_t *_dst, uint8_t *_src, ptrdiff_t stride, int8_t *offset_table, int width, int height);
void sao_edge_filter_neon(uint8_t *_dst, uint8_t *_src, ptrdiff_t stride, SAOParams *sao, int width, int height, int c_idx);
#endif // SAO_ARM_NEON
#endif // HAVE_NEON
#endif // ARCH_ARM
////////////////////////////////////////////////////////////////////////////////
...
#ifdef SAO_ARM_NEON
	hevc_sao_band_filter_0_neon(dst, src, stride, offset_table, width, height);
#else
    for (y = 0; y < height; y++) {
        for (x = 0; x < width; x++)
                                    ...
                                    uint8_t *horiz_edge, uint8_t *diag_edge)
{
    ...
    stride /= sizeof(pixel);
#ifdef SAO_ARM_NEON
	sao_edge_filter_neon((uint8_t *)dst, (uint8_t *)src, stride, sao, width, height, c_idx);
#else
