...
extern int64_t g_m3u8_last_keyframe_pts; //in av_time_base
extern int g_m3u8_start_sequence;
char* g_input_m3u_filename = NULL;
char* g_output_m3u_filename = NULL;
...
//     }
	...
	{//roger
		if(avformat_seek_file(input_files[0]->ctx,
		                    -1, 
		                    INT64_MIN, 
		                    g_m3u8_last_keyframe_pts, //in AV_TIME_BASE
		                    INT64_MAX, 
		                    AVSEEK_FLAG_BACKWARD) < 0)
		{
		    av_log(NULL, AV_LOG_ERROR, "see to %lld failed\n", g_m3u8_last_keyframe_pts);
		    return;
		}
	}
...
static int m3u8_list(int64_t duration, char* outputfile)
{
	AVIOContext* m3u8Context = NULL;
	if (( avio_open2(&m3u8Context, outputfile, AVIO_FLAG_WRITE,
						  NULL, NULL)) < 0)
	{
		avio_closep(&m3u8Context);
		return -1;
	}

	avio_printf(m3u8Context, "#EXTM3U\n");
	avio_printf(m3u8Context, "#EXT-X-VERSION:3\n");
	avio_printf(m3u8Context, "#EXT-X-TARGETDURATION:%d\n", 10);
	avio_printf(m3u8Context, "#EXT-X-MEDIA-SEQUENCE:%d\n", 0);
	
	av_log(NULL, AV_LOG_INFO, "file:%s, duration: %d\n", outputfile, duration);

	char basename[1024] = {0};
    strcpy(basename, outputfile);
    char* p = strrchr(basename, '.');
    if (p)
        *p = '\0';
	
	int tsFileCount = (duration/AV_TIME_BASE + 1) /10 + 1;
	int i=0;
	for (i;i<tsFileCount;i++)
	{		
		avio_printf(m3u8Context, "#EXTINF:%d,\n", 10);
		avio_printf(m3u8Context, "%s%d%s\n", basename, i, ".ts");
	}

    avio_printf(m3u8Context, "#EXT-X-ENDLIST\n");
...
{
	return g_m3u8_last_keyframe_pts;
}

int m3u8_get_last_seq_id()
{
	return g_m3u8_start_sequence;
}
...
int m3u8_create_list(char* inputfile, char* outputfile)
{
	...
	g_input_m3u_filename = inputfile;
	g_output_m3u_filename = outputfile;
	
	AVFormatContext* ctx = avformat_alloc_context(); 
	if(avformat_open_input(&ctx, g_input_m3u_filename, NULL, NULL) != 0)
    {
        av_log(NULL, AV_LOG_ERROR, "open %s failed\n", g_input_m3u_filename);
        return -1;
	}
	if(ctx->duration <= 0)
	{
	...
	m3u8_list(ctx->duration, g_output_m3u_filename);
...
{
	g_m3u8_last_keyframe_pts = starttimestamp;
	g_m3u8_start_sequence = startsequence;

	int argc = 10;
	char** argv = {"",
		"-t", "120",
		"-i", g_input_m3u_filename,
		"-codec", "copy",
		"-vbsf", "h264_mp4toannexb",
		g_output_m3u_filename};
