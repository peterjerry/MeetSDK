...
typedef struct HLSContext {
    ...
    int cur_seq_no;
	int live_start_seq_no; // seekback new added
	int is_live;
    int first_packet;
...
static int hls_read_header(AVFormatContext *s)
{
	...
	// guoliang.ma
	if (strstr(s->filename, "?m3u8seekback=true") || strstr(s->filename, "&m3u8seekback=true"))
	{
		c->is_live = 1;
		struct variant *v = c->variants[0];
		if (c->live_start_seq_no == 0) {
			c->live_start_seq_no = v->playlists[0]->start_seq_no;
			av_log(s, AV_LOG_DEBUG, "set c->live_start_seq_no %d\n", c->live_start_seq_no);
		}
		s->duration = v->playlists[0]->segments[0]->duration * (v->playlists[0]->cur_seq_no - c->live_start_seq_no);
	}
...
static int hls_read_packet(AVFormatContext *s, AVPacket *pkt)
{
	...
	{
		struct variant *v = c->variants[0];
		int64_t new_duration = v->playlists[0]->segments[0]->duration * (v->playlists[0]->start_seq_no + v->playlists[0]->n_segments - c->live_start_seq_no);
		if (new_duration > s->duration)
                               ...
                               int64_t timestamp, int flags)
{
    ...
    if ((flags & AVSEEK_FLAG_BYTE) ||
        !(c->variants[0]->playlists[0]->finished || c->variants[0]->playlists[0]->type == PLS_TYPE_EVENT || c->is_live))
